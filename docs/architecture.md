...
Value / CarryとEarnings Powerは別のOpportunity Laneであり、同一scoreへ畳まない。Selection PolicyがLane内のnomination / ordering、Attention PolicyがLane間のallocation、Research Gateがresearch-worthiness judgment、人間がPrimary Research Setへのadmissionを所有する。current two-Lane wireは`longlist` / `longlist_alt.entries` / `review_tickers`であり、generic registry・executor・Dynamic Attention Composerは持たない。
