...
| `estat_dashboard` | 無認証 JSON API | JP 公式マクロのうち e-Stat DB が持たない系列（完全失業率 季節調整値・名目賃金指数） | 統計ダッシュボード（総務省統計局）の `getData`。1 IndicatorCode が月次/四半期/年 × 原数値/季節調整値/前月比を同時に返すため、`provider_series_id` は `<IndicatorCode>?Cycle=1&IsSeasonalAdjustment=<1\|2>&RegionCode=00000` で 1 本に固定し、provider が全行の `@cycle` / `@isSeasonal` / `@indicator` / `@regionCode` と宣言 total 行数を照合する（filter が効かなかった応答を混入させない）。読むのは月次のみ。毎月勤労統計は確報だけが載る |
