...
[`../doctrine.md`](../doctrine.md) §2 の単一ループを、工程ごとに 1 doc で辿る。各 doc は意味論 + 手順 + 最小例を持ち、成果物の機械契約は `records/_schemas/*.json` を正本にする。
...
| 割安 screening | [`screening.md`](./screening.md) | 全上場から割安ゾーンを機械抽出し candidates 事実を出す |
| 個別銘柄リサーチ | [`research.md`](./research.md) | FV・RR・期待利回りを見積もり、塩漬け耐性を確認して採否 / sizing を決める |
...
上流の運用方針は [`../portfolio-management.md`](../portfolio-management.md)、構造は [`../architecture.md`](../architecture.md)、密な参照仕様は [`../reference/`](../reference/) を見る。
