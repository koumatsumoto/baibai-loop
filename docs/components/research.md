...
Baibai-Loop 4 成分アーキテクチャの **(d) 個別銘柄リサーチ** の運用仕様。screened × view から選定した個別銘柄の深掘り packet 本体で、本計画 v1 の主戦場。全体構造は [`../architecture-v1.md`](../architecture-v1.md)、スクリーニング詳細は [`../screening/`](../screening/) を参照。

## 1. 役割

- `screened/` × `view/` から選定した個別銘柄について、**一時的割安の原因仮説・反対仮説・catalyst・crowding を深く分析**
- 採用判定 / 見送り / 保留を記録（packet）
- Micro track の分析層、4 成分統合の出力点

## 2. 選定プロセス（screened × view → 候補絞り込み）

本節は v1 アーキテクチャの中核。`screened` (ミクロ事実) × `view` (マクロ見解) の 2 軸統合を具体化する。
...
- 最新 view が古く、その後に重大 brief が出て gate 判定に影響する場合:
...
### 2.3 view が存在しない期間（Bootstrap 前）

- v1 運用 Day 1 は view/ が存在しない。この場合は `view/` の bootstrap を先に実施する必要がある
- bootstrap 手順: [`view.md`](./view.md) の Bootstrap 規則を参照
...
採用した research の front matter path は、`trades/` の `research_ref` で参照される:

```yaml
# trades/2026/04/2026-04-26-7203.md
---
research_ref: research/2026/04/2026-04-25-7203-valuation-mean-reversion-v1.md
...
---
```
...
- [`view.md`](./view.md): Macro gate source の仕様
