...
3. approved / deferred / rejected decisions は position sync で記録され、`baibai-loop-position benchmark` と `baibai-loop-screening` の forward 計測 (`playbook-cohorts` / `selection-ablation`) の input になる。
