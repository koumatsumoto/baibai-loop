...
- `attribution_targets` は outcome を evidence、macro_regime_gate、sizing、execution、playbook のどこへ帰属させるかを構造化する。
...
| マクロ逆風 | Macro regime gate 判定の誤り、または gate が途中で反転 |
...
| macro supportive | マクロ追い風が想定以上に効いた |
...
- **Macro regime gate の判定精度** を集計（追い風判定した銘柄の +15/+30 パフォーマンス、逆風判定した不採用銘柄のパフォーマンス）
