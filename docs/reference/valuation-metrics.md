...
- 開示の利益は期中累計なので、`直近累計 + 前期通期 - 前年同期間累計` で 12 か月へ直す。前年FY末と同期間の開始・終了は月末を保つ12か月前の日付で照合する。合成できない期は `null`（単一四半期で割った偽の割高 PER を作らない）
...
`normalized_per_3fy` は、as-of 以前に開示された直近 3 FY の EPS を現在の株式数基準へ分割補正し、その単純平均で現在株価を割る raw Derived Metric である。FYの連続性は月末を保って前年へ移した末日で確認するため、2/28と閏年の2/29は連続する。3 FY が揃わない、選択済みFYのEPSが欠ける、補正後 EPS 平均が正でない、または価格・分割係数を確定できない場合は `null` とし、別指標へのフォールバックは行わない。
...
- `dps_forecast_annual`: 進行期の予想年間 1 株配当。四半期開示の `FDivAnn`、本決算開示では進行期ガイダンスの `NxFDivAnn` を使う。分割を跨ぐ行では基準不明の正値を落とし、明示0は残す。
- `dividend_yield` は、鮮度を満たす非負の `dps_forecast_annual` があれば原則として `dps_forecast_annual / 直近終値` とする。ただしsplit-safeな正の実績年間配当があり、予想が実績の2倍を超える場合は実績へ倒す。明示0の実績も利回り0として観測済みと扱い、予想・実績とも欠ければ `null` とする。

**予想は実績より優先するが、優先できるのは最新の非null実績 DPS と同日以降に開示され、かつ正の実績 DPS の2倍以下のときだけである。** 実績年間 DPS を持つ最新行より前に開示された予想は使わない。2倍を超える跳ねは特別配当か未実現の還元転換かを機械入力から区別できないため、5年反復するcarryには実績を使う。実績が0または未観測なら、無配からの初配当や実績開示前の新規上場を消さないため予想を使う。会社が予想を取り下げた後も過去の予想を引き当て続けると、無配化した会社に当時の配当額の利回りが付き、reversion 項の上限（5%/年）を単独で超える carry を作る。同じ規則が判断面の carry と較正 panel の両方で 1 つの実装から効く。
...
| `dividend_basis` | 意味 |
| --- | --- |
| `forecast_annual` | 鮮度とspike条件を満たす非負の予想 DPS から出した。明示0を含む |
| `actual_reported` | 正規化済み年間実績を追加の支払別再計算なしで採用した。期間内factorが1の値、またはbasisに依存しない明示0 |
...
四半期の配当基準日はFY末からの月移動で導出し、FY末が月末なら移動先も月末、非月末なら同日を保つ（存在しない日は移動先の月末へ丸める）。これは実際の支払日を表すものではない。支払ごとの換算は、導出した基準日と corporate action の権利落ち日の暦日差が5日以内に並ぶ年度では行わない。日本の分割は権利落ちが基準日の前営業日、効力発生が基準日の翌日という形が定型で、store は権利落ち日しか持たないため、その配当が調整の前の株数で払われたのか後なのかを言えない。換算した値は、株式基準を持たない配当総額を自己株控除後株式数で割った値と突き合わせ、5% を超えて食い違えば答えない。その株数は提出者自身が EPS を出すのに使った期中平均株式数から 2 倍以上外れていれば per-share の分母に使わない（自己株式数の欄に株数そのものが入る開示があり、時価総額が桁で小さくなる）。

`dividend_split_factor` は既存の配当計算窓の累積factorが1以外のとき記録し、1なら`null`とする。すべての分割の有無を表すflagではない。
...
J-Quants の財務サマリーは四半期 disclosure の時系列として扱うため、直前 disclosure は YoY ではなく QoQ になる。`eps_yoy` / `sales_yoy` / `operating_profit_yoy` は前年同期間を優先し、期間metadataが不足する場合は同じ`TypeOfCurrentPeriod`と前年の`CurrentFiscalYearEndDate`で照合する。対応する前年値が無ければ`null`とする。nullは比較不能であり、改善や悪化なしを確認した値ではない。季節性によるQoQ変化をYoYへ読み替えない。
