...
**分母の基準が揃えば `price_to_equity` と `pbr` は一致する。** `bps` は J-Quants がこの基準で出しているので `pbr` 側は元から正しく、揃えるのは時価総額と `equity` の側である。

**残る差は BS の時点であって定義ではない。** `bps` は本決算開示にしか載らないので直近の非 null 行から carry-forward され、`equity_to_asset_ratio` / `total_assets` は四半期開示にも載る。両者が別の開示日から来る銘柄では、同じ概念を**別の決算期末**で測ることになる。as-of 2026-08-06 の実測:

| 両者の出どころ | 銘柄数 | \|乖離\| 中央値 | 乖離 > 1% |
| --- | ---: | ---: | ---: |
| 同一開示日 | 1,681 | **0.21%** | 5.0% |
| 別開示日 | 1,984 | 2.72% | 78.7% |

同一開示日で残る 0.21% は `EqAR` が小数第 3 位までで開示されることによる丸めである（自己資本比率が小さい銘柄ほど相対誤差が大きい）。**`price_to_equity` は四半期時点、`pbr` は本決算時点**の比率だと読む — screening gate（`price_to_equity_max`）には新しい方が入る。

**欠損を代用で埋めない。** 自己株式数が観測できない行は発行済のまま計算し、`shares_outstanding_basis` に `issued` を立てて読み手へ渡す（`issued` は自己株ゼロの証明ではない）。自己資本比率が観測できない行は `equity_ratio` を `null` にし、純資産比率で代用しない — 代用は少数株主持分の大きい銘柄で比率を数 pt 過大にし、`equity_ratio_min` の screening gate を通しやすくする向きに効く。発行済を超える自己株式数のような破損値も発行済へ落とす（差が 0 以下だと時価総額が消え、銘柄が流動性母集団から黙って抜ける）。
