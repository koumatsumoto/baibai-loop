...
読者は、一般的な金融・経済用語を理解する一方、repository の内部語や書き手独自の比喩は知らない人とする。目標は平易化や日本語化率の向上ではなく、対象・変化・因果・不確実性・投資上の含意を一読で判別できる文章である。次の6原則を正本とする。

1. **標準用語を使い、見出しを説明的にする。** 見出しは「対象 + 変化または状態」が分かる簡潔な名詞句にする。キャッチコピー、未定義の比喩や造語を使わない。`残差`は明示したモデルで定義した場合、`レジーム`は分類規則や転換条件を指す場合だけ使う。
2. **結論を先に置き、一文に一つの主張を書く。** 結論、根拠、限定・反証の順で書き、観測・比較・原因・結論・投資含意を一文へ詰め込まない。主語、対象、比較期間は判断に必要な範囲で明示し、名詞化で文を膨らませない。
3. **観測・解釈・投資上の含意を分ける。** `fact_summary`には観測、公表値、計算結果、比較だけを書き、`示唆する`や`可能性がある`で解釈を混ぜない。`judgment`は引用済みfactの統合評価、`transmission` / `economic_connection`は「何が、どの経路で、何へ、どちら向きに影響するか」、`counter_evidence`は弱める判断と範囲を書く。scenarioは成立条件・経済経路・含意、monitoringは観測対象・成立条件・変更する判断、connectionは見積り成分・候補タイプ・影響方向を明記する。
4. **因果と確度を証拠の強さに合わせる。** 直接確認は「上昇した／公表された」、複数factの統合評価は「高水準にある／下支えしている」、因果を識別できない推論は「整合的である／可能性がある」、未確認は「確認できない／特定できない」と書き分ける。この確度表現は主にjudgment、synthesis、connectionへ適用する。擬人化で因果を強めず、上流の未確認・推定・代理指標という限定をsummary以降も保持する。
5. **数値は主張に必要なものだけを、基準付きで書く。** 単位、観測日、比較期間、前年比・前月比・水準、real / nominal、金額 / 数量、percentileの実効窓を、判断に必要な範囲で添える。0〜1のpercentile値は、たとえば`0.983`を「10年窓の98.3パーセンタイル」と表示する。裸の数値や異なる基準の無説明な併記を避ける。数字を減らすこと自体は目的にしない。
6. **判断内容を変えない推敲にする。** 数値、日付、単位、比較期間、source ID、引用関係、direction、confidence、materiality、probability、scenario conditions、scorecard、falsifier、`applies_to`、`affected_component`、支持・反証関係、限定を変えない。語順、文の分割、標準用語、見出しは変更できる。新しい因果・評価・対象範囲が必要なら推敲ではなく分析変更としてsource reviewへ戻す。

用語は巨大なglossaryではなく、次の3群で判断する。

- GDP、CPI、PCE、PMI、PER、PBR、EPS、USD/JPY、JGB、OAS、DI、bp、バリュエーション、ポジションサイズ、キャリーなど、定着した金融・経済用語はそのまま使える。
- FV、E[r]などBaibai Loopの正準語は実装定義と一致させる。E[r]の`carry`は「E[r]のcarry成分（配当利回りと株数縮小による年率寄与）」、文脈が明らかなら「配当・株数縮小による寄与」と書き、利益成長を含めない。
- raw enum・内部code・無説明の英単語は自然文へ漏らさない。たとえば`base / bear / bull`は「中心／下振れ／上振れ」、`breadth`は「上昇銘柄比率」または「市場上昇の広がり」、`proxy`は「代理指標」とする。`tail`は一律に下振れとせず、「発生確率は低いが影響の大きい下振れリスク」「発生確率は低い上振れ要因」または具体的なevent名で方向と対象を示す。
