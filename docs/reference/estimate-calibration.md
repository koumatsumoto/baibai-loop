...
panel には全普通株（universe scope）を記録し、`in_population`（selection.liquidity 通過 = 流動性母集団）を評価の母集団 flag にする。`selection_rank` は liquidity と E[r] 非欠損を満たす母集団を E[r] 主キーで並べた本番順位、`recommended_rank` は research selection depth 内の推奨順位（深さ 50）。本番 rules は sector / playbook cap で E[r] 順位を崩さず、集中度は diagnostics と research / position policy で扱う。評価はこのほかに `er_ranked_topN`（pass_screen × er 非 null 集合の E[r] 降順・仮想 replay）と `er_population_topN`（screen gate なしの母集団選抜・gate の付加価値診断）を出力する。
