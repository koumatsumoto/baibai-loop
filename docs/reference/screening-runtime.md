...
`cloud-history-backfill` は pull 直後と backfill 終了後の `market.sqlite` SHA-256 を比較する。source failure があっても commit 済み chunk が増えた場合は `PRAGMA quick_check` 後に `publish-lake` → `push-market` の順で R2へ保存し、その後に元の非0を返す。fetch 由来 15 table は lake 所有なので、release へ載せる前の store は dehydrate に拒否される。storeが変わらないfailureはGB級objectを再uploadしない。再dispatchはR2へ保存済みのcoverage/rowsをpullするため、既存chunkを再取得しない。
