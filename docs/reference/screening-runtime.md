...
**決算ラグ annotation**: 決算開示と as-of 財務のラグを判断面へ出す。`jquants_earnings_calendar` は ticker あたり 1 行しか持たないので、発表当日はその行が as-of と同じ日付を指したまま残り、`next_earnings_date` だけでは「予定」と「発表済み」が区別できない。`next_earnings_status` が as-of との前後で `scheduled` / `announced` を分け、カレンダー行が無い ticker は過去の開示周期から推定した `next_earnings_estimated_date` を持つとき `estimated`、無ければ `unknown` になる。推定は前年同四半期の次回発表日を 1 年ずらしたもので、確定日を上書きしない（確定日を持つ 2,103 銘柄との突合で誤差 3 日以内 88%・7 日以内 96%・中央値 0 日。event risk 判定は確定日だけで行い、推定は着手順の目安に留める）。`fin_latest_disclosed_date` は機械行の財務が含む最後の開示日で、カレンダーが as-of 以前の発表を指しているのにこれがそれより前なら `stale_fin_flag` が立つ — 発表は済んだが store に取り込まれていない窓であり、その行の財務・FV アンカー・E[r] は旧四半期のままである。いずれも annotation で、screen pass・自動除外・E[r]・rank・recommendation を変更しない。カレンダー行を持たない universe ticker 数は `run` の進捗行に出す（実測で universe の約 16%。個別企業の未公表を含むので閾値は置かず、急増を provider 欠落として読む）。
