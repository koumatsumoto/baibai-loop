...
`backfill-history` は日次足・財務サマリー・営業日カレンダ・週次信用残高・報告空売り残高を、明示した窓に対して 1 回で取得する。`bootstrap-cache` は窓を as-of から導き、通常指標用の日次足 1200 日・財務サマリー 730 日に加えて、`normalized_per_3fy` の分割基準と 3 FY を確定する両 source の 2200 日 coverage を要求する。窓を 1 度名指しすれば 1 パスで済み、coverage が既存の窓と繋がる。source ごとに独立に取得して行ごとに結果を出し、1 つの失敗が残りを止めない。日次足・財務サマリー・空売り残高報告は暦年で区切って要求する。窓全体を 1 度に読むと 10 年分の行をメモリに載せることになるうえ、区切りを `--start` でなく暦に置けば、開始日の違う実行どうしが同じ chunk を再利用できる。日次足・財務サマリーとも被覆済みの chunk は skip するので、中断した実行は chunk 単位で再開する。被覆の判定材料は違い、日次足は保存行そのもの（DBが正本、後述 §11.1）、財務サマリーと空売り残高報告は `source_coverage` を読む。空売り残高報告は訂正を取り込むため直近7日だけ被覆済みでも再取得する。営業日カレンダは provider 呼び出し 1 回なので分割せず、再開の単位にもならない。日次足を先に取るのは、`backfill-master` の月末グリッドが bar store から導出されるためで、bars の無い月の snapshot はまだ要求できない。
...
`verify-cache-coverage` は SQLite が `run --asof` で必要な全入力をローカルに提供できるかを read-only で検証する。検証対象は J-Quants master、通常指標用の日次足 1200 日・財務サマリー 730 日、`normalized_per_3fy` 用の両 source 2200 日 coverage、asof の営業日カレンダ、JPX の決算発表予定 snapshot と規制 snapshot、rules の `universe.required_jpx_flags` に含まれる source 名、EDINET metrics。財務サマリーは日付 coverage と独立にrequired-field populationを検証し、各field、`market_cap_required_fields`、`valuation_required_fields` の ticker件数を常に出力する。日付rangeがcompleteでもrequired-field floor未満なら `required-field:<name>@<asof>` を具体的な不足件数とともに返し、候補を全件nullのまま組み立てる前にexit 1とする。決算発表予定は論理 source `jpx_earnings_calendar` が `ok`、保存行数と coverage 件数が一致して 1 件以上、実データの最大日が asof 以後、取得が asof から 7 平日以内であることを要求する。`--allow-stale-jpx` は取得時刻だけを緩和し、空・部分保存・全件過去は許可しない。master は requested as-of のexact rowとcanonical coverageだけを照合し、range、status、row count、common-stock populationの一致を要求する。newer/prior snapshotを代用せず、別日snapshotの破損もrequested dateの判定へ混ぜない。日次足は SQLite 実データの行そのものから completeness を判定し（DBが正本、後述 §11.1）、財務サマリーは source_coverage の窓で判定する。`normalized_per_3fy` の追加窓も同じ authority で検証し、不足時に `null` や短い履歴へ黙って縮退しない。いずれも ticker/date 密度を追加で確認する。EDINET metrics は常に必須であり、raw JSON の読み込みや provider API 呼び出しは行わず、schema migration も行わない。不足があれば exit 1。
...
- `jquants_all_issues_daily_margin(balance_date, ticker, ...)` — 主キー `(balance_date, ticker)`。2026-09-25 残高からの全銘柄日次開示。`margin_*` は 9/25 以後の balance date をここから読む。公式移行確認と ClientV2 実 payload 検証まで batch 取込も serving も無効で、週次 table と row を union しない。境界と運用契約は [`margin-publication-transition.md`](./margin-publication-transition.md)
...
- `edinet_documents(doc_date, sequence_number, doc_id, sec_code, doc_type_code, csv_flag, xbrl_flag, legal_status, disclosure_status, withdrawal_status, doc_info_edit_status, parent_doc_id, operation_datetime, submit_datetime, doc_description, period_start, period_end, edinet_code, issuer_edinet_code, subject_edinet_code)` — 主キー `(doc_date, sequence_number)`。`sec_code` は提出者の証券コードであり、大量保有・公開買付では提出者と対象会社が別なので、対象会社は `issuer_edinet_code` / `subject_edinet_code` で持つ。縦覧中（`legal_status IN ('1','2')`）の typed row が identity を持たなければ、その日の identity を「観測していない」として扱い、提出が無かった日と区別する（後述「資本配分・支配権イベントの typed fact」）。縦覧終了（`legal_status='0'`）row は再 list しても identity を得られず event にも使わないため、identity が NULL でも観測済みの窓を未観測へ戻さない。列は 4 つに分かれる。**記述列**（`sec_code` / `doc_type_code` / `parent_doc_id` / `submit_datetime` / `doc_description`）と **identity 列**（`edinet_code` / `issuer_edinet_code` / `subject_edinet_code`）は提出そのものの主張で、EDINET は縦覧期間が満了すると entry を返したまま伏せる。満了は「その属性が無かった」ではなく「もう配信されない」ことなので、再 list は両集合の保存済み非 NULL を NULL で上書きしない。lake の generation はその保持済み row を厳密な dataset 契約で運び、同じ主キーに別 payload を黙って合成しない。**lifecycle 列**（`legal_status` / `withdrawal_status` / `csv_flag` / `xbrl_flag`）は読取時点で EDINET がその書類をどう扱っているかであり、新しい読取をそのまま採る。残りの列も新しい読取を採り、`doc_id` が変わった位置には以前の記述・identity を持ち越さない
...
- `lake_store_origin(singleton, release_id, release_manifest_sha256)` — このSQLite generationをhydrateしたL1 releaseのexact identity。hydrateはcandidate DBへ書いてからdurable replaceし、publish/dehydrateはsidecarでなくこのrowをcurrent pointerと照合する。market factではなくstore-local publication metadataであり、mergeはcloud sourceから取り込まずtargetのrowを保持する。
...
- 既存 cache がある asof では長期履歴を再取得しない。日次足の coverage は行データから導出し（§11.1、DBが正本）、range fetch の coverage は overlapping / adjacent window と union merge する（§11.1）。chunk 境界が asof ごとにずれても、行が揃っていれば偽のギャップを作らず re-fetch しない。
...
**判断時の機械行焼き込み**: run store は3世代retentionで、あるShortlistをrankしたselectionはそのrunと一緒に消える。selectionのnew-writeは、select時のrulesからproduction method identityを独立再計算してsource runの`screening_rules_hash`へ一致させ、`er_model_version`、recommendation / longlistのcandidate membership、rank、native / 表示 E[r]、FV・価格・estimate snapshotをsource candidateへ束縛する。さらにValue / Carry originと各rowのPolicy hash、strict typed Selection / Attention parametersから再計算した両Policy hash、Review Setのmembership / order / capを保存transaction内で検証する。identityを持たないlegacy runや現行rulesと異なるrunからexact selectionは新規発行しない。Shortlist v5は`review_tickers`とentry集合をexactに束縛し、現行Value / Carry-only resolverは`review_tickers`が`longlist`の全tickerと順序まで一致する場合だけ受理する。source rowの`opportunity_lane_id` / `selection_policy_id` / `selection_policy_hash` / `lane_rank` / native value・unit / `baseline_er_rank` / `primary_evidence_pattern_id` / `policy_diagnostic_ids`を`machine_snapshot`へ焼き込み、publish後は再計算も上書きもしない。v4 historyは保存payloadを書き換えず、readerがLane=`value-carry`を`legacy_inferred`としてprojectするが、存在しなかったPolicy ID・hashは捏造しない。

**Research Gate → Primary Research Setの束縛**: `research prepare`は`--shortlist-id`でcanonical Shortlistを明示し、`selection_id` / as-of / run revision / Review Set membershipをfail-closeで照合したうえで、`selected`をadmission可能集合としてworkspaceへ固定する。Primary Research SetはShortlist `selected`の部分集合に限り、Review Setに居ても`rejected`ならthesis scaffold前に拒否する。admission可能集合の正本はstored Shortlistで、workspace manifestはその読み取り用記録にすぎない — 各gateはselection_idからDBの判断を再解決してmanifestと突き合わせる。judgmentはbound shortlistの名前ではなくselectionから引くので、shortlist IDを書き換えても別cycleのGateへ乗り換えられない。workspaceを手で書き換えて別のpublished selectionとその判断へ束縛し直すことは可能だが、その場合もadmitできるのはいずれかのpublished Research Gateが`selected`としたtickerに限られる。Research Gate判定への異議はworkspaceでoverrideせず、同じrunへ`select`を実行し直し、修正した判断でShortlistをpublishしてから再prepareする経路で扱う。Research Gate contractを持たないv2-v4 historyはread modelとしては読めるが、researchのbindingには使えない。holding reviewはledgerがsourceなのでこのShortlist束縛を持たないが、`purpose: holding_review`を宣言するだけでgateを外せないよう、その対象tickerを各gateでcanonical ledgerの保有と照合する — どちらのpurposeも、workspaceではなくstoreに対して対象を証明する。Bargain Assessment publishの`selected`検証はdefense-in-depthとして残す。

**non-Core Opportunity Laneをproduction Review Setへ入れる前の条件**: 現行wireはValue / Carry-onlyで、`foundation/review_set.py`と`screening/selection/contracts.py`が`review_tickers != longlist`をfail-closeする。non-Core Laneを追加するIssueは、次を同一PRで満たしてから採用する。(1) Review Setのcanonical source rowをAttention Policyのprecedenceどおり一意に解決できる、(2) Thesis scaffoldのscreening estimate transfer（現在はValue / Carry `longlist`を直読する）が同じresolved rowを読む、(3) Research Gate `selected`のnon-Core tickerが`research prepare → thesis-scaffold → assessment`までE2Eで到達する、(4) Review Set外 / Research Gate rejectedのtickerはresearchへ迂回できない、(5) overlapのsource precedenceをAttention Policy contractどおり扱う。generic Lane registry / executor / Composerは作らない。
