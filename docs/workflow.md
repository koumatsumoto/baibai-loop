...
`world-weekly` / `world-daily` / `macro-monthly` を新規作成する前に、既存 brief の時系列欠損を必ず確認する。AI agent の作業前チェックとして扱い、省略しない。
...
3 週連続同方向を「トレンド」と呼ぶと「トレンドが続く見込み」という含意を誘発するため、解釈を帯びる「連続トレンド」「転換点」のような語は使わず、純粋な計算結果として「方向履歴」「方向反転」を用いる。
...
初期閾値は FX / 株式 / 金利市場の一般的通念に基づく暫定値。**四半期ごと**（四半期末月の最終週次 brief 作成時）に過去 12 週の実データから標準偏差を算出し、Notable ≒ 1σ / Major ≒ 2σ を目安に閾値を校正する。校正結果は本ファイルの閾値テーブルに反映する。

## 前回値の扱い

- 「前回」列には前月値（月次指標）または前四半期値（四半期指標）を記載する
- 前回値も一次統計から取得する
- 予想値はエコノミスト予想の信頼できる公開一次ソースが限定的なため、テンプレートには列を設けない。予想に言及する場合は本文中に出典を明記する
