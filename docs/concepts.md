...
| 売買執行記録 (trades) | 注文・約定・保有・決済を記録する | 採用判断がどう約定・保有・決済されたか |
...
| 売買執行記録 | trades（execution record） | 執行 | L3 | 注文・約定・保有・決済の記録 |
| 判断・追跡レジスタ | decision register（ledger） | 記録 | L3 | 売買提案・執行の決定イベントを機械正規化する |
...
- **個別銘柄リサーチ (research)** は investment memo。evidence count だけでなく、entry・target・stop・expected upside / downside・risk/reward・time horizon・invalidation を検証する。
...
- **売買執行記録 (trades)** は execution record。実際に order / entry した採用判断を扱う。
- **判断・追跡レジスタ (ledger)** は研究判断と執行イベントを append-only に正規化する正本。Candidate は screen fact、trades は execution として分ける。
