...
Baibai-Loop は、自分自身の裁量判断に一貫性を持たせ、後から検証できるようにするための基盤である。投資助言サービスでも、運用を委任するシステムでもない。

## 1. 目的（このシステムで達成したいこと）

日々の暮らしの中で **お買い得な優良銘柄を探し、長期で積み立てる**（配当利回りがあればなお良い）。狙いの核心は「一時的に売られすぎた割安株を掴む」ことにあるが、売り時は **保有期間ではなく valuation（株価水準の割高・割安）で判断して全株売却**する。想定どおりに上がらなくても **塩漬けを許容できる銘柄だけを選ぶ**。

売りの引き金は 2 つだけに絞る：**(a) 割高化**（フェアバリュー到達・割高ゾーン入り）、**(b) 事業のファンダメンタルズ毀損**。**株価が下がったこと自体では売らない**（価格による損切りは置かない）。だからこそ、採用の時点で **塩漬け耐性**（ネットキャッシュまたは健全な財務・営業キャッシュフローの黒字・低い有利子負債・借換に耐える体力）を必須の関門にする（配当は加点材料であって必須条件ではない）。
...
- 突き合わせの母数は自分の保有の実現結果であり、件数は少ない。その代わり、一つひとつの保有を長期にわたって深く観測する。フィードバックが遅く件数が少ないことは、長期保有戦略の性質上の必然として受け入れる（銘柄全体を対象にした短期の backtest で件数を稼ぐ最適化はしない。柱 5）。
...
- **(a)** 主軸は、全上場銘柄の実データを保持する **データ層（L1）** と、決定論的な screen・軸別スコアからなる **分析層（L2）** であり、判断層（L3 = records）はその消費者にあたる（3 層の詳細は [`architecture.md`](./architecture.md)）。ここでの L2 の「分析」は決定論的な機械処理を指し、その出力（candidates・軸別スコア）は柱 1 の意味では **事実** に属する（柱 1 / §6 で「分析」と呼ぶのは人間 / AI の解釈 = macro context・thesis）。計測手段を持たない機械的機能は追加しない。計測の対象は **長期戦略が依存するもの**（見積り精度・実現利回り・valuation の収束）とし、**銘柄全体を対象にした短期の forward-backtest（replay / cohorts / ablation）は行わない**。
- **(b)** スコアは軸ごとの座標（業種相対・自己レンジ相対の percentile）であり、単一の合成点や売買指示には決して畳まない。正直な軸別の事実 + 人間の判断という役割分担が、AI の強み（機械可読な事実の整理・統合）を活かしつつ、弱み（判断の責任を負えないこと）を遮断する。
...
- 過去データに対する閾値の網羅探索（grid search）やパラメータ最適化、戦略の累積リターン（年率・最大ドローダウン・シャープレシオ）を実績として掲げること。
- 機械学習によるスコアリング・予測。スコアは軸別の座標として出し、合成点に畳まない。
- 自動発注・リアルタイム処理。
- 銘柄全体を対象にした短期の forward-backtest（replay / cohorts / ablation）による screen 最適化。
- 部分売却 / リバランスの schema 化（割高になれば全売りする）、ETF / 投資信託 / 海外株、口座・税制のモデル化。
