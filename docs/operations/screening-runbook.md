...
`select` は `headwind` 業種を除外した上で、lane-specific metric に基づき triage します。signal 数と時価総額だけで機械的に上位化しません。出力の `lane_toplists`、`selection_lane`、`selection_metrics` を見て、どの割安仮説を research するのかを決めます。flattened な `candidates` の件数は `--top`、lane 別一覧の件数は `records/_config/screening-rules.yaml` の `output.lane_toplist_limit` で調整します。
