...
戦略 / screening / judgment gate / playbook の forward return を **「いつ、どう測るか」** を統一する。本 doc は**手順と原則**を正本にし、dated 計測ダイジェストは `reports/<YYYY-MM-DD>-*.md` に残す。
...
6. **multi-axis**: 単一 axis では確証 / 反証が出ないため、最低 4 axis（regime / playbook / outcome / week）で交差させる
...
### Axis B — playbook cohorts（既存 CLI）

`baibai-loop-screening playbook-cohorts` で各 playbook の **全銘柄** の forward return を集計。recommended queue に乗らない playbook の cohort 品質を見る。

### Axis C — selection ablation（既存 CLI）

`baibai-loop-screening selection-ablation` で ranking 成分（fast_boost / long_hold / playbook_rank / strength / diversity 等）と各 playbook の `Δfull` を計測。死荷重の検出と playbook 順序の検証。
...
### Axis G — regime × playbook クロステーブル

`(playbook, regime) -> mean rel` のクロステーブルを出す。regime 別に playbook の頑健性が違う前提で観察する (2026-05 観測では valuation-reversion が rally に頑健、cashflow-yield が劣後など)。
...
| 2026-05 | playbook cohorts | `strict-net-cash` / `fcf-yield` playbook が top5 不到達、`cash-rich` が baseline +5.32pt で全 playbook 中最強 | `strict-net-cash` / `fcf-yield` 削除、`cash-rich` 維持 (PR #246) |
| 2026-06-17 | judgment-gate counterfactual / playbook × regime cross / bootstrap CI | approved 8 件 rel −3.45pt、95% CI [−7.60, +0.32]、P(<0)=96% | regime gate を judgment 層に実装 (PR #245) |
