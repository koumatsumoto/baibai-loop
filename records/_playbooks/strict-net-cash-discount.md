...
- EDINET source trace: `edinet_source_doc_id`、`edinet_document_type`、`edinet_source_submit_datetime`、`edinet_source_period_start`、`edinet_source_period_end` を確認し、対象期間が直近の thesis と一致するか確認する。半期報告書 / 訂正半期報告書では `edinet_source_period_end` が fiscal year end を指すことがあるため、BS の実際の基準日を一次資料で確認する。
