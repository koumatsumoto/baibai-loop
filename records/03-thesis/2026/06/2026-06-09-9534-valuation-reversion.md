...
candidate_ref:
  candidates_ref: records/02-candidates/2026/06/2026-06-08.yaml
  ticker: '9534'
...
entry_preflight:
  ...
  reason: '2026-06-09 約定の 100 株 starter。市場相対は 6/8 暴落日（日経 -3.85%）に対し candidate の price_change_1d −0.79% で +3.06pt、4
    週のセクター相対は ticker_return_4w −8.2% vs sector_return_4w −7.67% で −0.53pt とほぼ中立。9534 はnet debtであり、利上げによる支払利息と原油高による原料費調整lagのリスクを負う。さらに screening の evidence_hit は sector_rotation_short_sell（売りシグナル）で買い根拠では
...
- **Data 上の矛盾（must read）**: (i) screening は 9534 を **short 候補（sector_rotation_short_sell, 15.2%ile）** として出しており long の根拠ではない。(ii) 9534 は **net debt −418 億（時価比 −62.9%、有利子負債 512 億）** で、利上げによる支払利息とbond-proxyの評価低下に感応する。(iii) 原油高は原料費調整のlagを通じてマージンを圧迫し得る。#201 の「利上げ耐性」評価はbalance sheetと整合しない。
...
- **金利・原油感応**: net debtと原料費調整lagにより、利上げと原油高が利益率・評価へ与える影響は大きい。
...
ユーザー確認により、Issue #203 の注文は 100 株、742.0 円で全量約定した。100 株の実 notional は 74,200 円で、実資金 5,000,000 円比 1.48%、tactical budget 2,000,000 円比 3.71%。net debt、原油感応、screen shortのため、discretionary 100株starterに限定する。
...
| Market baseline | Nikkei 6/8 -3.85% | candidate row |
...
| Action | starter | net debt、原油感応、screen shortのため、discretionary 100株starterに限定 |
...
- binding cap: paper→real 21% scaled cap（210,000 円）内。net debt、利払い、原油感応、screen shortを再検証するまで追加買いしない。
