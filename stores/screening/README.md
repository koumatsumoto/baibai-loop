...
- market schema は v13 を初期基準とする forward-only migration で進化します。schema bump 時は既存 store を再取得なしで in-place に前進 migrate し、v13 未満・最新超だけを「削除して再取得」で fail-fast します
...
- market SQLite を作り直す場合は `bootstrap-cache --asof` と `extract-edinet-metrics --asof` で provider から必要 window を再取得します（[`../../docs/reference/screening-runtime.md`](../../docs/reference/screening-runtime.md)）
