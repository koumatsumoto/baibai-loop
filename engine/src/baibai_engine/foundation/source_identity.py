"""Identify what a build computed: its own source, and the release line that wrote it.
...
byte. Measured on the calibration panel's 79-file closure, 31.4% of the hashed bytes are
...
Parsing a closure costs 216 times what hashing the same bytes does, and a fingerprint is
taken once per published partition, so the digest is memoised. The key is the source text
itself rather than the path: identical bytes cannot parse to different trees, so a hit is
exact, and a file rewritten in place is a new key rather than a stale hit.
"""
...
__all__ = ["release_line", "semantic_source_digest"]
...
@cache
def _digest_of_source(source: str, filename: str) -> str:
    # The filename is carried only so a syntax error names the file it came from; two
    # copies of the same text under different paths cost one extra entry and agree.
    tree = _without_docstrings(ast.parse(source, filename=filename))
...
def semantic_source_digest(path: Path) -> str:
    ...
    return _digest_of_source(path.read_text(encoding="utf-8"), str(path))
