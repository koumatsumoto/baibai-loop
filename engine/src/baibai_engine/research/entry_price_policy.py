...
class EntryPricePolicyError(ValueError):
    """A thesis cannot produce a usable entry price."""


def maximum_acceptable_entry_price(thesis: ThesisDocument, *, tick_size_yen: Decimal) -> Decimal:
    ...
    scenario = next(
        (
            item
            for item in thesis.estimates.scenarios
            if item.horizon_years == 5 and item.name == "base"
        ),
        None,
    )
    if scenario is None:
        raise EntryPricePolicyError("thesis has no 5y/base scenario")
    ...
    if terminal_shares <= 0:
        raise EntryPricePolicyError("5y/base terminal share count must remain positive")
    ...
    required_return = Decimal(str(thesis.estimates.required_5y_base_cagr_pct)) / 100
    ...
    if rounded <= 0:
        raise EntryPricePolicyError("max acceptable price must remain positive after tick rounding")
    return rounded


__all__ = ["EntryPricePolicyError", "maximum_acceptable_entry_price"]
