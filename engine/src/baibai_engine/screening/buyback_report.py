...
# 形で、金額の側は正しい。境界はこの空白に置く。
#
# 固定値でよいのは、これを超える価格帯がこの様式を出さないからである。終値 10 万円超は
# 58 銘柄あるが全て REIT と ETF で、自己株券買付状況報告書は 1 行も無い。普通株で提出する
# 銘柄の最高は 68,185 円 (6273)。決議単価 (`resolved_amount_yen / resolved_shares`) との比で
# 判定する案は採らない — 決議は株数と金額を独立に決めるので比は市場価格を表さず、実測では
# 正しい行 (6078: 単価 1,406 円・当日終値 1,416 円) を決議単価 143 円との比 9.8 倍で落とす。
#
# 前提が崩れるのは 6273 型の高値銘柄が上限へ近づく場合で、そのときは run の fallback 行に
# 出る「読めず field を落とした報告」の件数が先に動く。
