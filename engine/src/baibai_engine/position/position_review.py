"""Position Review contract: thesis health and after-tax replacement drive action.

A Position Review draft decides ``hold / add / reduce / exit`` for one open
...
from baibai_engine.position.ledger import estimated_exit_tax_for_gain_yen
...
class PositionReviewError(ValueError):
    """Raised when a Position Review draft cannot be loaded."""
...
class PositionReviewSources(BaseModel):
...
class PositionReviewDocument(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_replacement_inputs(self) -> PositionReviewDocument:
...
def load_position_review(path: Path) -> PositionReviewDocument:
    try:
        raw = safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as error:
        raise PositionReviewError(f"failed to load position review: {error}") from error
    if not isinstance(raw, Mapping):
        raise PositionReviewError("position review root must be a mapping")
    try:
        return PositionReviewDocument.model_validate(raw)
...
def _validate_evidence_freshness(document: PositionReviewDocument, errors: list[str]) -> None:
