...
- `.bak`は1世代のみで、次のpushで置き換わる。日次batchが毎営業日pushするため、実質の巻き戻し猶予は約24時間である。registry編集後は日次workflowの`registry-prune`行（series ID・observation/provider-run削除件数）を当日中に確認する。意図しないpruneを検出したら、次のpushが`.bak`を置き換える前に復元する。
