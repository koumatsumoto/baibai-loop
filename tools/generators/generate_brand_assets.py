...
`web/frontend/brand/logo.png`, run this, and the whole set follows.
...
otherwise leave `web/frontend/public/` carrying the new mark while the palette and its measurement
still describe the old one, and no check downstream reads the images.

Run it with `uv run --script tools/generators/generate_brand_assets.py`. Pillow is declared
...
applied here live in `tools/generators/brand_mark.py`, which type checks and tests do reach;
