...
`tools/generators/generate_brand_assets.py` renders them, and it needs Pillow — which stays out
of the project's lock so no daily workflow installs a native image library. So the decisions live
...
DEFAULT_SOURCE = REPO_ROOT / "web/frontend/brand/logo.png"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "web/frontend/public"
# Written next to the source so the palette can be checked against the image it claims to come
# from: `web/frontend/tests/brand.test.ts` reads this, not the developer's memory.
...
def check_destinations(source_path: Path, output_dir: Path) -> None:
    ...
    # asset was rendered from. Neither is visible downstream, since
    # `web/frontend/tests/brand.test.ts` reads the measurement and never the images.
...
def measure_palette(histogram: Histogram, bands: Sequence[Band] = BANDS) -> dict[str, str]:
    ...
    by a step under a resample. `web/frontend/tests/brand.test.ts` therefore holds the palette
