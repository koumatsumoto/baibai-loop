...
SCHEMA_PATH = Path(__file__).resolve().parents[3] / "records" / "_schemas" / "thesis.json"
...
def load_thesis_document(
    path: Path,
) -> tuple[dict[str, object], str] | list[ValidationFinding]:
    return _load_thesis_document(path)


def validate_thesis_collection(
...
def discover_thesis_files(root: Path) -> list[Path]:
    if not root.exists():
        return []
    return sorted(p for p in root.rglob("*.md") if p.is_file())


def _load_thesis_document(
    path: Path,
) -> tuple[dict[str, object], str] | list[ValidationFinding]:
    try:
