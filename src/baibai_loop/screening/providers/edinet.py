...
class EDINETProvider:
    ...
    def __init__(self, api_key: str, cache_dir: Path, session: requests.Session | None = None) -> None:
    ...
    def list_documents(self, on_date: date) -> list[dict[str, Any]]:
        ...
        url = f"{EDINET_API_BASE}/documents.json?{urlencode({'date': on_date.isoformat(), 'type': 2, 'Subscription-Key': self._api_key})}"
        data = self._request_json(url)
        results = data.get("results", [])
        if not isinstance(results, list):
            raise EDINETProviderError("EDINET documents.json returned unexpected results payload")
        cache_path.parent.mkdir(parents=True, exist_ok=True)
        cache_path.write_text(json.dumps(results, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    ...
    def load_metric_records(self, asof_date: date) -> dict[str, EdinetMetricRecord]:
        ...
        return {record.ticker: record for record in (normalize_metric_record(item) for item in payload)}
    ...
    def _request_json(self, url: str) -> dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                raise EDINETProviderError(f"EDINET request raised: {type(exc).__name__}: {message}") from None
            if response.status_code < 400:
                return response.json()
            if response.status_code not in {429, 500, 502, 503, 504}:
                raise EDINETProviderError(f"EDINET request failed with status {response.status_code}")
...
def normalize_metric_record(record: Mapping[str, Any]) -> EdinetMetricRecord:
    ...
    return EdinetMetricRecord(
        ...
        ttm_quality_ev_ebitda=_parse_ttm_quality(_coalesce(record, "ttm_quality_ev_ebitda", "TTMQualityEvEbitda")),
        ttm_quality_p_s=_parse_ttm_quality(_coalesce(record, "ttm_quality_p_s", "TTMQualityPS")),
        ttm_quality_pcfr=_parse_ttm_quality(_coalesce(record, "ttm_quality_pcfr", "TTMQualityPCFR")),
    )
