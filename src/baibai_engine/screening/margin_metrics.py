...
# of. Its long/short ratio is not a large number; it is undefined, and mixing the
# two into one cross-section would rank instruments against a quantity half of
# them cannot have.
...
class MarginSupplyDemand:
    ...
    # The classic margin ratio. High means the long side is crowded relative to
    # the short side. Defined only where a short balance is possible at all.
...
def margin_supply_demand(
    *,
    ...
    Every axis is None when its inputs are missing rather than defaulted, because
    a zero balance and an unobserved balance mean opposite things: the first says
    nobody is positioned, the second says nobody looked.
    """
    if latest is None:
    ...
    return MarginSupplyDemand(
        ...
        margin_long_to_adv=(
            ...
            if long_vol is not None and avg_daily_volume_shares
            else None
        ),
        margin_ratio=(
            long_vol / short_vol
