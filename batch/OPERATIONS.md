...
`record_count`は行が在る場所でしか証明できない。hydrate済みのtargetでは実rowに対して証明し、R2が運ぶdehydrate済みのsourceでは証明せずclaimとして受け取る。cleanな財務summary rangeのtarget側countはunion後の実rowから再生成し、再検証する。sourceのclaimを落とせば再取得が1回増えるだけだが、実体の無いclaimを作れば穴が隠れるので、危険なのは後者の向きだけである。**target側の証明はhydrate済みであることのgateでもある**——空のtargetは「取得済み」と言うclaimを黙って0へ書き換える代わりに、ここで停止する。
