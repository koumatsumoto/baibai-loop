      ...
      <section className="grid gap-3">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <SectionCard description="E[r] rankingによるResearch Gateの入力母集団" padded title="RankedSet">
            <div className="grid gap-3">{data.selections.length === 0 ? <p className="text-sm text-muted-foreground">RankedSet はありません</p> : data.selections.map((selection) => <div className="rounded-lg border p-3" key={selection.selection_id}><div className="mb-2 flex flex-wrap gap-2"><span className="font-mono text-xs text-muted-foreground">{selection.selection_id}</span></div><div className="flex flex-wrap gap-2">{selection.ranked_set.map((item, index) => <Badge key={String(item.ticker ?? index)} variant="secondary">{String(item.ticker ?? 'unknown')}</Badge>)}</div><p className="mt-2 text-xs text-muted-foreground">{selection.ranked_set.length} 件</p></div>)}</div>
          </SectionCard>
        </div>
      </section>
