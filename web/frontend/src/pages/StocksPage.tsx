...
export function StocksPage() {
  ...
  return (
    <PageShell
      ...
      meta={<div className="flex flex-wrap gap-4"><UpdatedAtBadge value={data.run.run_at} /><AsOfBadge compact value={data.run.asof_date} /><span className="font-mono text-xs text-muted-foreground">分析 {data.run.analyzed_security_count.toLocaleString('ja-JP')} 銘柄</span></div>}
      title="Stocks"
    >
      ...
      <SectionCard description="Research Set の thesis を比較した資本配分判断" padded title="Capital Allocation Assessment">
        {data.capital_allocation_assessments.length === 0 ? <p className="text-sm text-muted-foreground">Capital Allocation Assessment はまだ publish されていません。</p> : <div className="grid gap-2">{data.capital_allocation_assessments.map((item) => <Link className="rounded-md border p-3 hover:bg-muted/40" key={item.capital_allocation_assessment_id} to={`/stocks/capital-allocation-assessments/${item.capital_allocation_assessment_id}`}><div className="flex flex-wrap items-center gap-2"><Badge>{item.result}</Badge><strong>{item.headline}</strong>{item.allocated_ticker && <span className="font-mono">{item.allocated_ticker}</span>}</div><p className="mt-1 text-xs text-muted-foreground">基準 {item.as_of} · alternatives {item.alternative_count}</p></Link>)}</div>}
      </SectionCard>
      <ReviewSetSection reviewSet={latestReviewSet} runAsOf={data.run.asof_date} />
      <SecurityAnalysisSection rows={data.security_analyses} runAsOf={data.run.asof_date} />
    </PageShell>
  )
}
