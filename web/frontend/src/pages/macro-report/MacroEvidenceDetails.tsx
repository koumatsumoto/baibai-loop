...
function CoreEvidence({ section, headingLevel = 3, judgmentShown = false, historyShown = false }: { section: MacroCoreSectionView; headingLevel?: 3 | 4; judgmentShown?: boolean; historyShown?: boolean }) {
  ...
  return (
    <div className="grid gap-5">
      {!judgmentShown && <div className="grid gap-2"><div className="flex flex-wrap items-center gap-2"><Heading className="font-semibold">Judgment</Heading><JudgmentBadge confidence={section.judgment.confidence} direction={section.judgment.direction} /></div><p>{section.judgment.summary}</p><SourceIds ids={section.judgment.source_ids} /></div>}
      ...
      {(section.material_deltas ?? []).length > 0 && <div className="grid gap-3 border-t pt-4"><Heading className="font-semibold">Material delta</Heading>{section.material_deltas.map((delta, index) => <article className="grid gap-2 rounded-lg bg-muted/45 p-3" key={`${delta.channel}-${index}`}><div className="flex flex-wrap gap-2"><Badge variant="outline">{delta.channel}</Badge><Badge variant="secondary">{labelMacroValue(delta.direction)} / 重要度 {labelMacroValue(delta.materiality)}</Badge></div><p>{delta.summary}</p><p className="text-xs text-muted-foreground">使用先: {delta.used_for}</p><SourceIds ids={delta.source_ids} /></article>)}</div>}
      <div className="grid gap-2 border-t pt-4"><Heading className="font-semibold">Economic Connection</Heading><p>{section.economic_connection.summary}</p><SourceIds ids={section.economic_connection.source_ids} /></div>
      {(section.fact_summary ?? []).length > 0 && <div className="grid gap-3 border-t pt-4"><Heading className="font-semibold">Fact</Heading>{section.fact_summary.map((fact, index) => <div key={`fact-${index}`}><p>{fact.summary}</p><SourceIds ids={fact.source_ids} /></div>)}</div>}
      {(section.series ?? []).length > 0 && <div className="grid gap-2 border-t pt-4"><Heading className="text-xs font-semibold text-muted-foreground">参照 series</Heading><SeriesReferences series={section.series} /></div>}
    </div>
  )
}
...
export function MacroEvidenceDetails({ partition }: { partition: MacroCorePartition }) {
  ...
  return (
    <SectionCard description="Fact・Judgment・Economic Connection と参照 series / source ID を、必要な経済チャネルだけ開いて確認できます。" padded title="詳細な経済チャネルと証拠">
