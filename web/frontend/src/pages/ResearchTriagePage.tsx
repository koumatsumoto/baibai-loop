...
export function ResearchTriagePage() {
  ...
  return (
    <PageShell lead="Review Set を読んだ人間が Research / Skip と、その理由・調査質問・主要リスクを記録した面です。最終的な buy 判断ではありません。" meta={<div className="flex flex-wrap gap-4 font-mono text-xs text-muted-foreground"><AsOfBadge value={triage.as_of} /><span>{formatJstDateTime(triage.published_at)}</span><Badge variant="secondary">{triage.research_triage_id}</Badge></div>} title="Research Triage">
