...
export function CapitalAllocationAssessmentPage() {
  ...
  return (
    <PageShell lead="Research Set の thesis を同じ時点で比較し、資本を配分するか見送るかを記録した判断です。" meta={<div className="flex flex-wrap gap-3 text-xs text-muted-foreground"><Badge>{data.result}</Badge><span>{data.as_of}</span><span>{formatJstDateTime(data.published_at)}</span></div>} title="Capital Allocation Assessment" width="reading">
