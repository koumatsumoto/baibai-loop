...
統括（現状計測の解釈・仮説の事前登録・design/confirm 判定・採否レビュー）と実務（panel 構築・`calibration-build` / `calibration-evaluate` の実行・variant rules の機械適用）を分離できる場合、実務はサブエージェントに委譲し、統括者が返ってきた計測結果を主要原則と事前登録した採否基準に照らしてレビューする。委譲しても採否の判断と事前登録の責任は統括者が持ち、サブエージェントの結論は検証してから採用する（同時起動は最大 5）。
