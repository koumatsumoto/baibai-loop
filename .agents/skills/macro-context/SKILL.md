...
1. **入力を固定する**
   ...
   `ops-maintenance` skill に従い market / runs / macro を pull し、market を hydrate する。application DB は pull しない。head は ID と as-of だけを読み、trigger は前回結論を含まない投影結果だけを読む。
