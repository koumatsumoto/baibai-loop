...
thesis authorと別roleで実行し、thesisを直接編集しない。

## Input

- longlistとshortlist比較
- thesis path/core hash
- source URL、公表日、対象期
- `baibai-engine research evaluate <thesis>`の`five_year_base_break_even`
...
6. base terminal multipleが観測値を上回る場合、premiumを支えるthesis内fact IDと一次source IDを記録し、`decision_impact`で維持または修正理由を説明したか。数値の余裕だけを根拠にしない。
...
10. limit/max price/quantity/notionalがthesisと一致するか。
...
decision-review schemaのdraftと、更新した既存research checklistだけを返す。break-even計算、観測anchor、または一次情報によるpremium根拠を確認できない場合はcheckを`blocked`にし、理由と必要なthesis修正を`decision_impact`へ書く。scenarioまたはproposalを変える場合は`proposal_changed=true`としてthesis authorへ戻す。thesis変更後は新hashにreviewを再生成する。repo外資産配分、hedge、単元未満株等へscopeを広げない。
