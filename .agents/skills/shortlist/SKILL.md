...
7. **annotation 消化**（不変条件: 判断面へ annotation を足す変更は、この表へ消化規則を同時に足す）:
   ...
   | `buyback_remaining_share_ratio` / `buyback_remaining_amount_ratio` / `buyback_trailing_3m_acquired_ratio` / `buyback_authorization_window_end`（同じく `longlist[].buyback_authorization`。`buyback_report_month_end` がこの 4 つの基準日で、提出日とは 2 週間から 1 か月ずれる） | 様式 220 が月次で出している取得枠の中身。**carry を「これから受け取る現金」として narrative に書いてよいかは、ここで決まる**。**枠は「取得し得る株式の総数」と「取得価額の総額」の 2 本を上限に持ち、先に尽きた方で取得が終わる。残枠は 2 つのうち小さい方である** — 決議後に株価が上がった銘柄は金額枠を先に使い切って株数枠を残すので、株数側だけを見ると枠が残っているように見える（as-of 2026-08-10 の実測で、残枠比率を出せた 978 銘柄のうち 290 銘柄が「株数残 10% 以上かつ金額 95% 以上消化」、うち 53 銘柄は取得期間も未満了）。`buyback_remaining_share_ratio` / `buyback_remaining_amount_ratio` は決議した株数・金額のうちまだ使っていない割合で、どちらかが 0 に近ければ枠は使い切られており、`recent_filing` でも forward の還元は無い（6088 は 2026-08-05 提出＝齢 0 日だが、残枠 11%・取得期間は 7/31 満了）。`buyback_trailing_3m_acquired_ratio` は直近 3 報告月の取得株数 ÷ 発行済で、**carry の trailing 株数変化にまだ現れていない取得を拾う**（枠を持つ 365 銘柄のうち 219 銘柄は carry ≤ 0 だった）。`buyback_authorization_window_end` が as-of より前なら、直近の提出があっても枠はもう無い。**`null` は「読めなかった」であって「残っていない」ではない** — 様式の記載形式は filer ごとに揺れ、実測で残枠が読めたのは 8 割である。単独で自動除外・rank 変更に使わない | <!-- drift: allow-unrelated-policy-literal -->
   | `dividend_basis`（`selection.yaml` の `longlist[].dividend_basis`。`annual_yield` / `dps_forecast_annual` / `dps_actual_annual` / `basis` / `split_factor`） | carry のうち配当側が反復する普通配当かを判断する材料。**特別配当は予想年間 DPS へそのまま入る**ので、`dps_forecast_annual` が `dps_actual_annual` から桁で跳ねている行は一次開示（短信の配当表）へ戻って普通配当と特別配当を分ける。実測では forecast/前期実績が 3.0 倍以上は 3,109 銘柄中 15 件（0.5%）と稀だが、carry が E[r] の主キーなので**上位へ集中して現れる**（as-of 2026-08-14 で 3659 は forecast 475 円 / 前期実績 45 円 = 10.56 倍、うち 415 円が基準日 9/30 の特別配当で、反復利回りは 15.72% でなく 1.99% だった）。跳ねの向きは両方あり、減配後の復配・記念配当の剥落も同じ形をとるので**単独で自動除外・rank 変更に使わない**。`basis` が `forecast_annual` なら予想側、`actual_reported` / `actual_record_date_resolved` なら実績側を利回りに使っている。`dps_actual_annual` の `null` は分割を跨いで株式基準を確定できなかった年度で、無配と読み替えない |
   ...
   | 直近 cycle で棄却済み（`bargain_assessment` の reject / defer lane） | 深掘りを終えた ticker が翌 cycle も上位へ戻るのは E[r] 主キーの正常な挙動だが、**新しい材料が無いまま research 枠を再消費しない**。`baibai_engine.research_watch` の row と当日 selection の candidate rowを突き合わせ、次のいずれかを満たすときだけ再開候補にする: (a) `current_close_yen <= thesis_fair_value_yen` (b) 新規の material 開示がある (c) 前回の unknown 軸を解消する決算が出た (d) raw close が前回 thesis の entry 価格から **5%以上下落**した（`current_close_yen <= thesis_entry_price_basis_yen * 0.95`）(e) 機械財務の最新開示日が thesis の市場断面より新しい（candidate の `fin_latest_disclosed_date > rows[].thesis_as_of`）。(d) は終値同士を比べ、場中安値や前日比では発火させない。(e) は「新しい決算が存在する」triggerであって内容が改善したという判定ではないため、一次開示を読んでからselected / rejectedを決める。どれも無ければ既定で rejected（`reject_class: event_wait`）とし、reason に前回結論の日付と再評価 trigger を書く。selected にするなら**前回結論から何が変わったか**を narrative に明記する。`research_watch` が unresolved、または比較する日付・価格が `null` の軸は未発火として扱い、別triggerが成立したと推定しない |
   | E[r] 履歴帯（較正 quintile 文脈） | 帯の記述統計としてのみ参照。個別銘柄の予測として書かない |
   | `deterioration_unmeasurable`（`event_warnings` / `risk_tags`） | 悪化ゲートは「観測できた YoY のどれかが閾値以下か」で判定するので、`eps_yoy` / `sales_yoy` / `operating_profit_yoy` が 3 つとも欠測の銘柄は判定材料を持たないまま通る。**通過は「悪化していない」ことの観測ではない。** 該当は screen を通った流動性母集団の 0.9%（as-of 2026-07-31 で 4/433。母集団だけなら 7.9%、screen 通過だけなら 3.2%）。carry 支配型や資産型でこの tag が立ったら、直近の決算短信で減収・減益の有無を一次開示で確かめてから narrative を書く。**単独で自動除外にしない** |
...
8. **OP3 深度契約**: selected 各銘柄について次を 1 項目ずつ機械的に突合する（印象で「満たしているはず」としない）。
   ...
   6. carry 支配型で、`forecast_special_gain_flag`・FCF の配当カバーを剥がしても成立するか。carry の 2 成分は機械行でなく**一次開示で確定する**（機械の DPS / share-change は基準年ズレと遅れの両方を持つ）:
      - **配当**: 短信の配当表から特別配当を差し引いた普通配当が反復分。前期実績も同じ処理をしてから比較する。`actual_record_date_resolved` は機械が支払ごとに組み直した値なので、carry 支配型では短信の配当表へ戻って基準を確かめる。**株式基準を確定できなかった銘柄は機械行に現れない**（利回りが出ないので E[r] が付かず、longlist の手前で落ちる）。該当銘柄は selection の `counts.er_missing_unresolved_dividend_basis` に code で並ぶので、carry 支配型を探すときはこの一覧も見る — 無配でも割高でもなく、分割・併合を跨いだ年度で基準が確定できなかっただけの銘柄である
