...
7. **annotation 消化**（不変条件: 判断面へ annotation を足す変更は、この表へ消化規則を同時に足す）:
   ...
   | `deterioration_unmeasurable`（`risk_tags`） | 悪化ゲートは「観測できた YoY のどれかが閾値以下か」で判定するので、`eps_yoy` / `sales_yoy` / `operating_profit_yoy` が 3 つとも欠測の銘柄は判定材料を持たないまま通る。**通過は「悪化していない」ことの観測ではない。** 母集団の 12.4%、screen 通過の 7.1% が該当する。carry 支配型や資産型でこの tag が立ったら、直近の決算短信で減収・減益の有無を一次開示で確かめてから narrative を書く。**単独で自動除外にしない** |
