...
7. **plan-limit と proposal**:
   ...
   **starter band**: 5y base が要求 8.5% に届かないが 7.0% 以上で、永久損失が `elevated` でなく dated catalyst がある lane は、全件見送りの代わりに縮小 lot で建てる選択肢がある。thesis の `judgment` に `position_intent: starter` / `sizing_action: reduced` / `starter_catalyst_date` を置き、`estimates.required_5y_base_cagr_pct` をその lane で要求する水準（7.0 以上 8.5 未満）にする。`plan-limit` が 1 注文の金額を上限で切り、`proposal create` が帯と bucket 上限を検証する。条件と撤退基準は [`portfolio-management.md`](../../../docs/portfolio-management.md#starter-band) が正本。7 軸・独立レビュー・human override は緩めない。
