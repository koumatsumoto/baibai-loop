...
```bash
uv run python -m tools.measure_signal_cohorts --basis price --out .cache/838-price.yaml
uv run python -m tools.measure_signal_cohorts --basis total --out .cache/838-total.yaml
```
...
#833 の診断（[2026-08-06](./2026-08-06-bargain-capture-diagnosis.md)）が §9 で予測した「price-only は差を過小に出す方向であり、結論の向きは変わらない」は、4 軸中 3 軸で確認された。残る 1 軸（3.3 株数減少の継続性）は差が縮むが、もともと cohort 一致が半数付近で結論を担っていない。
