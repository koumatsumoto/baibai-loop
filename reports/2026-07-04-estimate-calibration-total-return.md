...
[baseline（price-only・2026-07-03）](./2026-07-03-estimate-calibration-baseline.md) の後続。配当（DPS）を cache に取り込み、超過リターンの一次基準を「配当 accrual 込み total return の母集団中央値対比」へ更新した再計測記録。**以後の計測・#295 の事前登録検証はこの total return 基準を正とする**。
