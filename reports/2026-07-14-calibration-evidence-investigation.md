...
| SQLite | `data/screening/market.sqlite` | master / bars / financial summaries |
...
UV_CACHE_DIR=/tmp/uv-cache uv run baibai-loop-screening calibration-build \
  --start 2022-09-01 \
  --end 2026-06-30 \
  --sqlite-path data/screening/market.sqlite \
...
`data/screening/calibration` は panel / forward を各46件持つが、現行 reader が要求する
