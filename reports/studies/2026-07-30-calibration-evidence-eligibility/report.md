...
```bash
sqlite3 'file:stores/market/market.sqlite?mode=ro' \
...
- `stores/market/market.sqlite` の既存行（§5.1 の `backfill-master` は断面 master を append しただけで、既存 snapshot・bars・fin・coverage は変更していない。cohort 再構築の検証は copy 上で実施）
