...
[`build_universe`](../../../engine/src/baibai_engine/screening/universe.py)は各tickerの最新raw closeとsplit-normalized sharesから時価総額を作り、`market_cap_oku = round(raw_close * shares / 1e8)`と整数億円に丸める。[`_screening_reference_close_yen`](https://github.com/koumatsumoto/baibai-loop/blob/51f1f3f253107e81bc53403cea35319452b7ba62/engine/src/baibai_engine/screening/selection/summaries.py)は、その丸め済み値を`round(market_cap_oku * 1e8 / shares, 4)`で株価へ戻す。この不可逆なround-tripが通常の差の原因である。

一方、[`estimate_expected_return`](../../../engine/src/baibai_engine/screening/estimates.py)が使うFV anchorとE[r]は丸め前のraw closeから作られる。[`build_selection_payload`](https://github.com/koumatsumoto/baibai-loop/blob/51f1f3f253107e81bc53403cea35319452b7ba62/engine/src/baibai_engine/screening/selection/payload.py)のaudit rankは`er_annual`降順、playbook順、evidence strength、tickerで確定した後に表示用`market_price_yen`を付与する。したがってreferenceをraw closeへ差し替えてもproduction audit rankとtop-20 membershipは変わらず、影響件数はcontract上0である。FV-gap順の仮想rerankはproductionに存在しないため行わない。
...
| `stores/market/market.sqlite` | raw bars 2021-08-02..2026-07-14 | `user_version=12`, 5,219,443 bar rows | exact-date raw close / factor |
