...
母集団 1,349・中央値リターン −2.86%（YAML −0.028571 ✓）／ PBR 軸 n=1,037・最安 decile median excess −4.67%（YAML −0.046704 ✓）／ 本番推奨 top-5 median excess −12.05%（YAML −0.120513 ✓）。分割正規化は 1:2 分割で asof-basis リターンが +20% になる unit test（`tests/engine/test_calibration_grid_forward.py`）で固定。
