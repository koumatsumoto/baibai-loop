...
_BANDIT = "uv run bandit -c pyproject.toml -q -r src/baibai_engine src/baibai_app tools"
...
def test_node_audit_is_triggered_by_the_lockfiles_it_reads() -> None:
    ...
    lockfiles = ["ui/package-lock.json", "cloud/worker/package-lock.json"]

    assert set(triggers) == {"pull_request"}
    assert set(jobs) == {"audit"}
    # Named exactly, not merely covered: `ui/**` would also satisfy a containment
