...
metric no playbook emits falls to its default for every candidate and orders nothing while
still looking like a tiebreaker, which is how one of them went unnoticed. These tests
assert the direction of each surviving element against the metrics its own playbook emits.
"""
...
class EvidenceStrengthKeyTest(unittest.TestCase):
    def test_cash_rich_orders_the_thicker_cash_pile_first(self) -> None:
        richer = _evidence_strength_key(PLAYBOOK_CASH_RICH, {"cash_to_market_cap": 0.8})
        thinner = _evidence_strength_key(PLAYBOOK_CASH_RICH, {"cash_to_market_cap": 0.4})
        self.assertLess(richer, thinner)

    def test_every_cash_rich_element_is_fed_by_the_playbook(self) -> None:
        # The playbook's own metric map, so a key element that reads something else
        ...
        self.assertNotEqual(
            _evidence_strength_key(PLAYBOOK_CASH_RICH, emitted),
            _evidence_strength_key(PLAYBOOK_CASH_RICH, {}),
        )

    def test_cashflow_yield_orders_the_higher_yield_and_then_the_stronger_growth(self) -> None:
        higher = _evidence_strength_key(PLAYBOOK_CASHFLOW_YIELD, {"ocf_yield": 0.2, "cfo_yoy": 0.0})
        lower = _evidence_strength_key(PLAYBOOK_CASHFLOW_YIELD, {"ocf_yield": 0.1, "cfo_yoy": 0.9})
        self.assertLess(higher, lower)
        growing = _evidence_strength_key(
            PLAYBOOK_CASHFLOW_YIELD, {"ocf_yield": 0.2, "cfo_yoy": 0.5}
        )
        shrinking = _evidence_strength_key(
            PLAYBOOK_CASHFLOW_YIELD, {"ocf_yield": 0.2, "cfo_yoy": -0.1}
        )
    ...
    def test_sales_discount_orders_the_deeper_gap_then_growth_then_profitability(self) -> None:
        deeper = _evidence_strength_key(
            PLAYBOOK_SALES_DISCOUNT, {"ps_sector_gap": -0.8, "sales_yoy": 0.1}
        )
        shallower = _evidence_strength_key(
            PLAYBOOK_SALES_DISCOUNT, {"ps_sector_gap": -0.5, "sales_yoy": 0.9}
        )
    ...
    def test_a_key_element_that_no_playbook_emits_would_order_nothing(self) -> None:
        ...
        for playbook in (
            ...
            with self.subTest(playbook=playbook):
                self.assertEqual(
                    _evidence_strength_key(playbook, {"a_metric_no_playbook_emits": 1.0}),
                    _evidence_strength_key(playbook, {}),
                )
