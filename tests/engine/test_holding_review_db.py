...
def test_db_holding_review_build_and_publish_recheck_canonical_revisions(
    ...
    published = ResearchStoreService(db, clock=lambda: FIXED_NOW).publish_holding_review(
    ...
    with pytest.raises(ValueError, match="source changed"):
        ResearchStoreService(db, clock=lambda: FIXED_NOW).publish_holding_review(
    ...
    assert (
        position_main(
            ["holding-review", "--db", str(db), "--input", str(draft)],
            now=FIXED_NOW,
        )
        == 2
    )
...
def test_holding_publish_revalidates_inside_write_transaction(
    ...
    real_validate = research_store_module._validate_holding_review_scalars_in_transaction

    def recording_validate(
        review_document: HoldingReviewDocument,
        *,
    ...
    ResearchStoreService(db, clock=lambda: FIXED_NOW).publish_holding_review(
        "holding-review-20260714-2331-transaction",
        THESIS_ID,
        document.model_dump(mode="json"),
    )
...
def test_holding_review_accepts_post_close_decision_at_current_operation_time(
    tmp_path: Path,
...
def test_expired_holding_degrades_without_changing_invalidation_or_action(
    ...
    before = build_holding_review_from_db(
        db_path=db,
        holding_thesis_id=THESIS_ID,
        position_id="2331",
        now=expires_at - timedelta(microseconds=1),
    )
    expired = build_holding_review_from_db(
        db_path=db,
        holding_thesis_id=THESIS_ID,
        position_id="2331",
        now=expires_at,
    )
    after_expiry = build_holding_review_from_db(
        db_path=db,
        holding_thesis_id=THESIS_ID,
        position_id="2331",
    ...
    ResearchStoreService(db, clock=lambda: expires_at).publish_holding_review(
        "holding-review-20260731-2331-expired",
        THESIS_ID,
...
def test_holding_publish_rejects_draft_that_expired_after_build(
    ...
    review_id = f"holding-review-expired-{publish_now.timestamp()}"

    with pytest.raises(ValueError, match="load-bearing values"):
        ResearchStoreService(db, clock=lambda: publish_now).publish_holding_review(
            review_id,
            THESIS_ID,
            document.model_dump(mode="json"),
        )
    ...
    with sqlite3.connect(db) as connection:
        assert (
            connection.execute(
                "SELECT COUNT(*) FROM holding_review WHERE holding_review_id = ?",
                (review_id,),
            ).fetchone()[0]
            == 0
        )
...
def test_holding_review_rejects_naive_operation_clock_before_write(tmp_path: Path) -> None:
    db = _database(tmp_path)
...
def test_holding_build_and_publish_forward_one_operation_instant(
    ...
    real_classify = holding_builder_module._classify_current_thesis_eligibility
    ...
    ResearchStoreService(db, clock=lambda: operation_now).publish_holding_review(
        "holding-review-20260714-2331-clock",
        THESIS_ID,
        document.model_dump(mode="json"),
    )
