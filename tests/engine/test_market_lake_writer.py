...
def test_daily_coverage_delta_limits_comparison_to_refetched_months(tmp_path) -> None:
    sqlite_path = _market_store(tmp_path / "market.sqlite")
    mirror = tmp_path / "mirror"
    base = _export(
        dataset_name="jquants.daily_bars",
        ...
        mirror_root=mirror,
        producer_git_commit=_COMMIT,
        build_id="coverage-base",
    )
    source = base.manifest.partitions[0].sources[0]
    assert isinstance(source, lake_models.SQLiteSnapshotSourceRef)
    fetched_at = (source.captured_at + timedelta(seconds=1)).isoformat()
    with sqlite3.connect(sqlite_path) as connection:
        connection.execute(
            "UPDATE jquants_daily_bars SET close = 111.0 WHERE traded_at = '2026-02-02'"
        )
        connection.execute(
            """INSERT INTO source_coverage(
                 source, coverage_key, coverage_start, coverage_end,
                 fetched_at_utc, record_count, status, error
               ) VALUES ('jquants_daily_bars', 'test:delta', '2026-02-01',
                         '2026-02-28', ?, 1, 'ok', NULL)""",
            (fetched_at,),
        )

    with writer_module._open_immutable(sqlite_path) as connection:
        assert writer_module._coverage_delta_periods(
            connection,
            require_lake_dataset("jquants.daily_bars"),
            base.manifest,
        ) == ((2026, 2),)


def test_invalid_coverage_delta_falls_back_to_full_comparison(tmp_path) -> None:
    sqlite_path = _market_store(tmp_path / "market.sqlite")
    mirror = tmp_path / "mirror"
    base = _export(
        dataset_name="jquants.daily_bars",
        ...
        mirror_root=mirror,
        producer_git_commit=_COMMIT,
        build_id="coverage-fallback-base",
    )
    with sqlite3.connect(sqlite_path) as connection:
        connection.execute(
            "UPDATE jquants_daily_bars SET close = 111.0 WHERE traded_at = '2026-02-02'"
        ...
        connection.execute(
            """INSERT INTO source_coverage(
                 source, coverage_key, coverage_start, coverage_end,
                 fetched_at_utc, record_count, status, error
               ) VALUES ('jquants_daily_bars', 'test:invalid', '2026-01-01',
                         '2026-02-28', 'invalid', 1, 'ok', NULL)"""
        )
