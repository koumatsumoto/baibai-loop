"""Position Review contract tests."""
...
def _load(name: str) -> PositionReviewDocument:
...
def test_file_source_binding_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="entity_id"):
        PositionReviewDocument.model_validate(raw)


def test_broken_thesis_is_priority_exit() -> None:
    result = evaluate_position_review(_load("thesis-break.yaml"))
...
def test_underwater_intact_thesis_holds() -> None:
    result = evaluate_position_review(_load("underwater-hold.yaml"))
    assert result.computed_action == "hold"
...
def test_after_tax_superior_candidate_reduces() -> None:
    result = evaluate_position_review(_load("replacement-superior.yaml"))
...
def test_confirmed_exit_tax_is_used_as_a_cash_flow() -> None:
    ...
    result = evaluate_position_review(PositionReviewDocument.model_validate(raw))
...
def test_unknown_tax_holds_and_shows_sensitivity() -> None:
    result = evaluate_position_review(_load("tax-unknown-hold.yaml"))
    assert result.computed_action == "hold"
...
def test_position_review_build_cli_writes_a_validated_draft(
