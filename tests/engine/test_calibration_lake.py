...
from baibai_engine.market.lake.keys import current_l2_pointer_key, pin_key
from baibai_engine.market.lake.models import DatasetManifest
...
class TestImmutableBuilds:
    def test_a_cohort_is_published_as_a_build_the_pointer_names(self, tmp_path: Path) -> None:
        ...
        for dataset in (CALIBRATION_PANEL, CALIBRATION_DIAGNOSTICS):
            ...
            assert manifest.source_release_ids  # a build always names its L1 input
...
class TestRetention:
    ...
    def test_a_pinned_build_survives_the_sweep(self, tmp_path: Path) -> None:
        ...
        create_pin(
            ...
            target_kind="l2_build",
            target_id=first.build_id,
            dataset=CALIBRATION_PANEL.name,
        ...
        assert [pin.pin_id for pin in read_pins(tmp_path)] == ["pin-adopted-cohort"]
    ...
    def test_a_pin_that_names_no_dataset_for_a_build_is_refused(self, tmp_path: Path) -> None:
        with pytest.raises(ValueError, match="must name its dataset"):
            create_pin(
                tmp_path,
                pin_id="pin-no-dataset",
                target_kind="l2_build",
                target_id="some-build",
                reason="reason",
                owner="owner",
            )
...
class TestReviewRegressions:
    ...
    def test_a_manifest_replaced_under_the_pointer_is_refused(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(CalibrationCacheError, match="digest does not match the pointer"):
            read_panel(tmp_path, date.fromisoformat(_JANUARY))
